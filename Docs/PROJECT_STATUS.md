# MoneyTransferService — Project Status
...
## Account identity / IBAN decision

Decision: transfer API uses IBAN, not account database ID.

Why:

- Account database ID is internal persistence identity.
- IBAN is external account identity for transfer requests/responses.
- Keep DB relations explicit with `SenderAccountId` / `ReceiverAccountId`.
- Store `SenderIban` / `ReceiverIban` on transactions as transfer-time snapshots.

Pattern:

```text
CreateTransferRequest(SenderIban, ReceiverIban, ...)
  -> TransferCommand(SenderIban, ReceiverIban, ...)
  -> lookup accounts by IBAN with update lock
  -> persist transaction FK by account ID
  -> persist transaction IBAN snapshots
```
...
Do not interrupt current feature work for these unless user explicitly asks.
Track refactor ideas here as they appear.
Prefer small separate refactor tasks after feature behavior is stable and tested.

Known refactor candidates:

1. Split `TransactionService` responsibilities later:
   - Keep transaction resource/query behavior in `TransactionService`.
   - Move transfer operation orchestration into a transfer use-case/service when deposit/withdraw work starts to crowd the class.
   - Avoid doing this during concurrency correctness work.

2. Clean naming inside `TransactionService`:
...
3. Split large business test file later:
   - `TransferServiceTests.cs` should become transaction-service/use-case oriented when test split is done.
   - Transfer operation test method names can keep transfer wording.

## Next likely tasks

1. Finish transfer concurrency correctness work:
   - UnitOfWork transaction boundary.
   - Optimistic concurrency retry.
   - Idempotency race handling.
   - Tests for retry/exhausted retry/idempotency race.
